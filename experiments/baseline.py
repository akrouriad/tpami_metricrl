        ...
        for env in envs:
            for norma in normas:
                for aggreg_type in aggreg_types:
                    run_name = env + 'sampIt' + str(ts_per_iter) + 'nb_v' + str(nb_v) + 'norma' + norma + 'aggreg' + aggreg_type + 'mts' + str(max_ts / 1e6) + 'm' + 'run' + str(run)
                    params = {'envid': env, 'seed': run, 'log_name': dir_out+run_name, 'max_ts': max_ts, 'norma': norma, 'aggreg_type': aggreg_type, 'nb_vfunc': nb_v, 'min_sample_per_iter': ts_per_iter}
