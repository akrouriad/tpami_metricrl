...
from cluster_weight_proj import ls_hardcweight_mean_proj
    ...
    while True:
        ...
        for epoch in range(nb_epochs_clus):
            for batch_idx in dat.next_batch_idx(batch_size_pupdate, iter_ts):
                ...
                cw_optim.zero_grad()
                eta = ls_hardcweight_mean_proj(HardClusterMembership.apply, policy_torch.get_cweights(), policy_torch.exp_dist(obs[batch_idx]),
                                               old_cweights, old_means[batch_idx], old_cov_d['prec'], kl_cluster, old_cmeans)
                policy_torch.cweights = eta * policy_torch.get_cweights() + (1 - eta) * old_cweights
        ...
        eta = ls_hardcweight_mean_proj(HardClusterMembership.apply, policy_torch.get_cweights(), policy_torch.exp_dist(obs),
                                       old_cweights, old_means, old_cov_d['prec'], kl_cluster, old_cmeans)
        cweights = eta * policy_torch.get_cweights() + (1 - eta) * old_cweights
        for k, cwp in enumerate(policy_torch.cweights_list):
            cwp.data = cweights[[k]]
        policy_torch.update_clustering()
        final_kl = proj.mean_diff(policy_torch.get_weighted_means(obs), old_means, old_cov_d['prec'])
