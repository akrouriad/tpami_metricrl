...
from metric_rl.metricrl_rt_pi import ProjectionSwapMetricQRL
...
def experiment(env_id, horizon, gamma, n_epochs, n_steps, n_steps_per_fit, n_episodes_test, seed, params,
    ...
    def tanh_action(input):
        a_dim = mdp.info.action_space.shape[0]
        input[-a_dim:] = torch.tanh(input[-a_dim:])
        return input

    critic_params = dict(preproc=tanh_action, input_shape=input_shape,
                         **params['critic_params'])
    params['critic_params'] = critic_params

    agent = ProjectionSwapMetricQRL(mdp.info, **params)
...
if __name__ == '__main__':
    ...
    log_name = generate_log_folder(env_id, 'projection_randcov_rt', str(n_clusters), True)
    ...
    Parallel(n_jobs=n_jobs)(delayed(experiment)(env_id=env_id, horizon=horizon, gamma=gamma, n_epochs=1000, n_steps=3000,
                                                n_steps_per_fit=3000, n_episodes_test=25, seed=seed, params=params,
