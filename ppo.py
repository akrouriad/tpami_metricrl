...
import rllog
from rl_shared import MLP, RunningMeanStdFilter, ValueFunction, GaussianPolicy
...
def learn(envid, seed=0, max_ts=1e6, norma='None', log_name=None):
    ...
    while True:
        ...
        # update policy and v
        adv = rl.get_adv(v_func=value, obs=p_paths['obs'], rwd=rwd.numpy(), done=p_paths['done'], discount=discount, lam=lam)
        ...
        v_target, _ = rl.get_targets(value, p_paths['obs'], rwd.numpy(), p_paths['done'], discount, lam)
        torch_targets = torch.tensor(v_target, dtype=torch.float)

        logging_ent = policy_torch.entropy()
        logging_verr = F.mse_loss(value(obs), torch_targets)
...
if __name__ == '__main__':
    ...
    learn(envid='BipedalWalker-v2', max_ts=1e6, seed=0, norma='None', log_name='test_idp')
    # learn(envid='RoboschoolHopper-v1',  max_ts=1e6, seed=0, norma='None', log_name='test_hop')
