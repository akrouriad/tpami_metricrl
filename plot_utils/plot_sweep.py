...
def plot_data(x, color, use_median):
    if use_median:
        median = np.median(x, axis=0).squeeze()
        epochs = range(len(median))
        plt.plot(epochs, median, color)
        plt.fill_between(epochs, np.quantile(x, axis=0, q=.25), np.quantile(x, axis=0, q=.75), alpha=.5, color=color)
    else:
        mean, interval = get_mean_and_confidence(x)
        epochs = range(len(mean))
        plt.plot(epochs, mean, color)
        plt.fill_between(epochs, mean - interval, mean + interval, alpha=.5, color=color)


def create_figure(res_folder, subfolder, name, env, all_data, alg_labels, line_color,
                  use_median=False, legend=False, display=False):
    plt.figure()
    legend_lab = []
    for n, d, lc in zip(alg_labels, all_data, line_color):
        d = np.array(d)
        plot_data(d, lc, use_median)
        legend_lab.append(n)
...
if __name__ == '__main__':
    xp_name = 'baseline_sweep'
    envs = ['HopperBulletEnv-v0', 'Walker2DBulletEnv-v0', 'HalfCheetahBulletEnv-v0', 'AntBulletEnv-v0']
    nb_epochs = 1000

    alg_labels = ['PPO - eps 0.05', 'PPO - eps 0.1', 'PPO - eps 0.2', 'PPO - eps 0.3', 'TRPO - max kl 0.005', 'TRPO - max kl 0.01', 'TRPO - max kl 0.015', 'TRPO - max kl 0.02']
    line_color = ['salmon', 'red', 'peru', 'brown', 'paleturquoise', 'teal', 'cyan', 'aquamarine'] 

    res_folder = './Results/'
    exp_folder = os.path.join(res_folder, xp_name)
    baseline_folder = os.path.join(res_folder, 'baselines_sweep')

    nb_runs = 25
    baselines_algs = ['ppo2_0.05', 'ppo2_0.1', 'ppo2_0.2',  'ppo2_0.3', 'trpo_mpi_0.005', 'trpo_mpi_0.01', 'trpo_mpi_0.015', 'trpo_mpi_0.02']

    count = 0
    for env in envs:
        all_j, all_r = load_data_baselines(baseline_folder, env, baselines_algs, nb_runs, nb_epochs, add_prop_id=False)

        print('creating figure in folder {}, subfolder plots/{}, for env {}'.format(res_folder, xp_name, env))
        create_figure(res_folder, xp_name, 'R', env, all_r, alg_labels, line_color,
